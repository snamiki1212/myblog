...
export const selectRelatedEntryList = ({
  ...
  return R.pipe(
    ...
    (list) =>
      R.filter(
        list,
        (candidate) => candidate.data.category === target.data.category,
      ),
    (list) =>
      R.filter(
        list,
        (candidate) => !ignoreEntrySlugList.includes(candidate.data.slug),
      ),
...
export const toPickedEntryInfoList = (
  entires: BlogEntry[],
  opt: { pageSize: number },
): PickedEntryInfo[] => {
  const [latestEntries, remainingEntires] = R.splitAt(entires, opt.pageSize);
  const obj = remainingEntires.reduce(
    (prev, current) => {
