  ...
  ({isPost, postNode}: {isPost: boolean; postNode: MarkdownRemark}): any => {
    ...
    const {title, description, image, postURL, category} = isPost
      ? {
          ...
          image: urljoin(
            config.siteUrl,
            (postNode.frontmatter.cover as any).childImageSharp
              ? (postNode.frontmatter.cover as any).childImageSharp.fluid.src
    ...
    const article = isPost
      ? {
          ...
          '@type': 'NewsArticle',
          //
          dateModified: postNode.frontmatter.updatedAt,
          datePublished: postNode.frontmatter.createdAt,
...
export const SEOMeta: React.FC<Props> = ({postNode, isPost = false}) => {
